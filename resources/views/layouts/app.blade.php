        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-0 shadow-xl flex flex-col justify-between">
            <div>
                ...
                <nav class="mt-4 px-2 space-y-1 text-sm font-medium">
                    <a href="{{ route('dashboard') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-slate-800 transition {{ request()->routeIs('dashboard') ? 'bg-emerald-600 text-white' : 'text-gray-300' }}">
                        <span>📊 ዳሽቦርድ (Dashboard)</span>
                    </a>

                    @if(Auth::user()->role && Auth::user()->role->name === 'admin')
                    <a href="{{ route('students.index') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-slate-800 transition {{ request()->routeIs('students.*') ? 'bg-emerald-600 text-white' : 'text-gray-300' }}">
                        <span>👨‍🎓 የተማሪዎች ዝርዝር</span>
                    </a>
                    <a href="{{ route('classes.index') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-slate-800 transition {{ request()->routeIs('classes.*') ? 'bg-emerald-600 text-white' : 'text-gray-300' }}">
                        <span>🏫 የክፍል ደረጃዎች</span>
                    </a>
                    <a href="{{ route('sections.index') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-slate-800 transition {{ request()->routeIs('sections.*') ? 'bg-emerald-600 text-white' : 'text-gray-300' }}">
                        <span>🗂️ ሴክሽኖች</span>
                    </a>
                    <a href="{{ route('subjects.index') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-slate-800 transition {{ request()->routeIs('subjects.*') ? 'bg-emerald-600 text-white' : 'text-gray-300' }}">
                        <span>📚 የትምህርት አይነቶች</span>
                    </a>
                    @endif

                    <a href="{{ route('marklist.index') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-slate-800 transition {{ request()->routeIs('marklist.*') ? 'bg-emerald-600 text-white' : 'text-gray-300' }}">
                        <span>📝 ውጤት ማስገቢያ</span>
                    </a>
                    <a href="{{ route('attendance.index') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-slate-800 transition {{ request()->routeIs('attendance.*') ? 'bg-emerald-600 text-white' : 'text-gray-300' }}">
                        <span>📅 የዕለት አቴንዳንስ</span>
                    </a>

                    @if(Auth::user()->role && in_array(Auth::user()->role->name, ['admin', 'finance']))
                    <a href="{{ route('finance.dashboard') }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-slate-800 transition {{ request()->routeIs('finance.*') ? 'bg-emerald-600 text-white' : 'text-gray-300' }}">
